...
void createLevel(int w, int h, int maxInsetMove, int edgeThresh, int mult) {
  ...
  for (int mountain=0; mountain<numRanges; mountain++) {
    ...
    int numSteps = int(random(1, 60*mult));
  ...
  for (int forest=0; forest<numForests; forest++) {
    int tx = int(random(0, w));
    int ty = int(random(0, h));
    int numSteps = int(random(10, 1500*mult));
    for (int step=0; step<numSteps; step++) {
      level[ty][tx] = FOREST;
      tx += int(random(-2, 2));
      tx = constrain(tx, 0, w-1);
  ...
  for (int pond=0; pond<numPonds; pond++) {
    int tx = int(random(0, w));
    int ty = int(random(0, h));
    int numSteps = int(random(10, 100*mult));
    for (int step=0; step<numSteps; step++) {
      level[ty][tx] = SEA;
      tx += int(random(-2, 2));
      tx = constrain(tx, 0, w-1);
  ...
  for (int y=0; y<h; y++) {
    for (int x=0; x<w; x++) {
      if (level[y][x] == BEACH) {
        testTiles.clear();
        hitLand = false;
        checkForIsolatedTiles(x, y);

        if (!hitLand) {
          for (PVector p : testTiles) {
            level[int(p.y)][int(p.x)] = SEA;
          }
        }
      }
    }
  ...
  // change beach back to tan when done
  for (int y=0; y<h; y++) {
    for (int x=0; x<w; x++) {
      if (level[y][x] == -1) {
        level[y][x] = BEACH;
...
void checkForIsolatedTiles(int x, int y) { 
  testTiles.add(new PVector(x, y));        // add to ArrayList

  // if we've hit something other than the beach, we're done
  if (level[y][x] != BEACH) {
    if (level[y][x] == GRASS || level[y][x] == FOREST || level[y][x] == ROCKS || level[y][x] == MOUNTAIN) {  // hit some land in the process, set true
      hitLand = true;
    }
    return;
  }

  // check all neigbors so long as we're not going offscreen
  else {
    level[y][x] = -1; // set to new color when traversed (prevents traveling the same pixels many times)

    // test neighboring pixels
    if (y-1 >= 0) {
      checkForIsolatedTiles(x, y-1);  // u
    }
    if (x+1 < w) {
      checkForIsolatedTiles(x+1, y);  // r
    }
    if (y+1 < h) {
      checkForIsolatedTiles(x, y+1);  // d
    }
    if (x-1 >= 0) {
      checkForIsolatedTiles(x-1, y);  // l
    }
    return;
  }
}
